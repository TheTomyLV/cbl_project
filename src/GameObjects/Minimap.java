...
public class Minimap extends GameObject {
    ...
    protected void draw(Graphics2D g2d) {
        ...
        int panelW = scene.getWidth();
        int panelH = scene.getHeight();
        ...
        for (GameObject go : playersLocal)  playerPositions.add(go.position);
        for (GameObject go : playersRemote) playerPositions.add(go.position);

        ArrayList<Vector2> enemyPositions = new ArrayList<>();
        for (GameObject go : enemies) enemyPositions.add(go.position);
